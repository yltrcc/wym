...
public class AdminController {
	...
	@RequestMapping("/toAuditHousePage")
	public String toAuditHousePage() {
		return "auditHouse";
	}
	 ...
	 * @return
	 */
	@RequestMapping("/findPassHouse")
	@ResponseBody
	public UserHouseData findPassHouse(int page, int limit) {
		Page p = new Page();
		p.setLimit(limit);
		p.setPage((page - 1) * limit);
		List<House> findAllHouse = service.findPassHouse(p);
		UserHouseData data = new UserHouseData();
		data.setCode(0);
		data.setCount(findAllHouse.size());
		data.setData(findAllHouse);

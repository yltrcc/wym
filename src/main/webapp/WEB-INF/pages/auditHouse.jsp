...
<div class="layui-fluid" style="margin-top: 40px;">
    <fieldset class="layui-elem-field layui-field-title">
        <legend style="font-size: 26px">待审核房源信息</legend>
    </fieldset>
       ...
       var dt = table.render({
        ...
        table.on('tool(house)',function(obj){
            ...
            if(layEvent ==="edit"){
                ...
                layer.confirm('确认当前数据审核通过吗？',{icon:5,shade:0.1},function(index){
                    $.post("passAuditHouse",{hID:data.hID},function(success){
                        if(success == "OK"){
                            obj.del();
                            dt.reload({
                                page:{curr:1}
            ...
            if(layEvent === 'delete'){
            	console.log(data.hID)
                layer.confirm('确认当前数据审核不通过吗？',{icon:5,shade:0.1},function(index){
                    $.post("noPassAuditHouse",{hID:data.hID},function(success){
                    	if(success == "OK"){
                    		obj.del();
                    		dt.reload({
                    			page:{curr:1}
...
<script type="text/html" id="tools">
    ...
    <a class="layui-btn layui-btn-xs layui-btn-danger" lay-event="delete">不通过</a>
</script>
</body>
</html>